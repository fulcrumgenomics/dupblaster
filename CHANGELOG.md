...
- **Duplicate marking is roughly 3% faster**, with byte-identical output, metrics and duplicate flags. Two hot-path fixes a profile pointed at, on a 19.7M-template name-sorted WGS run with every metric on: mate-tag CIGAR rendering no longer goes through `core::fmt`, and QNAME grouping now compares each record against the block's own first record instead of keeping a separate copy of the current QNAME in step. Measured 25.45 s → 24.70 s median over six runs each (mean 25.34 s → 24.69 s), on a machine whose run-to-run spread is about 1%. The bulk of a run is BGZF decompression on the worker thread, which neither change touches.
