...
- **Duplicates are now split into a sequencing and a library component, on by default.** A **sequencing** component (copies made on the flowcell — cluster/ExAmp duplicates) and a **library** component (PCR copies, plus genuinely distinct molecules that happen to share a locus). Two duplicates imaged on the same tile are copies of one molecule; on different tiles they are independent. Adds `sequencing_duplicates`, `library_duplicates`, `frac_sequencing_duplicates`, `naive_sequencing_duplicates`, `tile_count` and `tile_collision_rate` to `--stats`, and writes a per-sequencing-unit table beside it (`<STATS>.sequencing-units.tsv`) with per-flowcell/lane template counts, tile counts and sequencing-duplicate rates. Both-ends-mapped pairs only. Pass **`--no-sequencing-dups`** to turn it off — it only ever *classifies* duplicates, so turning it off never changes which reads are marked. Validated against read-name ground truth on a 333M-template EM-seq sample: 62.78% of duplicate pairs sequencing, against 62.73% from an independent reference implementation.
  - `--read-name-format <FORMAT>` names the layout: `illumina` (the default — `instrument:run:flowcell:lane:tile:x:y`, CASAVA 1.8+, bcl2fastq, BCL Convert), `element` (Element AVITI, the same layout), or `regex:<PATTERN>` with `(?<su>…)` and `(?<tile>…)` capture groups for platforms without a preset. The layout is never guessed beyond that default, because mis-parsing one would silently produce a confident wrong number. A read name it cannot parse is a **hard error if you named the format** and a **warning that skips the split if you didn't**, so MGI, Ultima, pre-CASAVA-1.8 and SRA-renamed inputs still process normally. Names that parse and then stop are always an error — that means two platforms' data got merged.
  ...
  - Measured on a 333M-template BAM: **+7.2% wall time**, +5.9% CPU, **no change in peak memory**, and 4.95 GiB of temporary disk — 16 bytes per pair, about 5 GB for a 30× human genome, under `--tmp-dir`. Only about a third of that (27 ns per pair) falls in the main pass; the other two thirds are the post-pass, which runs after the output stream is closed and so does not hold up a downstream process — the overhead a pipeline actually feels is roughly **+2.5%**. A failed spill write is a hard error rather than a silently dropped metric, and a startup check warns (but does not abort — the estimate swings with the input's compression) when the temp volume looks short. The temp spill is read back only after the output BAM is closed, so a downstream process is never left blocked on our stdout.
